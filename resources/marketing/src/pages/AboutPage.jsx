...
const AboutPage = () => {
  ...
  const focusCards = cards.length > 0
    ? cards.slice(0, 2)
    : [
        {
          title: 'Our Mission',
  ...
  const storyParagraphs = Array.isArray(story.body_paragraphs)
    ? story.body_paragraphs.map((paragraph) => paragraph?.text || paragraph).filter(Boolean)
    : [];

  return (
    <div className="bg-background pb-16 pt-24 text-foreground md:pt-32">
      <Seo
      ...
      <header className="next-container next-section-padding text-center">
        <motion.div {...fadeInProps()} className="mx-auto max-w-3xl">
          ...
          <h1 className="mb-6 mt-4 text-4xl font-semibold tracking-tight text-balance sm:text-5xl md:text-6xl">
      ...
      <section className="next-section-padding">
        <div className="next-container">
          ...
          >
              ...
              {storyParagraphs.length > 0 ? (
                storyParagraphs.map((paragraph, index) => (
                  <p
                    ...
                    className={`text-lg leading-8 text-muted-foreground ${
                      index === storyParagraphs.length - 1 ? 'mb-6' : 'mb-4'
                    }`}
                  >
                    {paragraph}
                  </p>
                ...
                <>
                  <p className="mb-4 text-lg leading-8 text-muted-foreground">
                    Founded with a passion for technology and a commitment to innovation,
                    TechTower began as a collective of builders focused on solving real
                    business challenges with dependable digital products.
                  </p>
                  <p className="mb-6 text-lg leading-8 text-muted-foreground">
                    We work closely with every client to understand the problem clearly,
                    shape the right solution, and deliver work that feels polished,
                    practical, and ready for growth.
                  </p>
                </>
              )}
            ...
            <div className="order-1 lg:order-2 overflow-hidden rounded-[2rem] border border-border/60 bg-muted/20 shadow-[0_24px_80px_rgba(0,0,0,0.16)]">
              <img
                src={
                  story.image_url ||
            ...
            {focusCards.map((card, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="next-card p-8"
                  {...fadeInProps(index * 0.08)}
                >
      ...
      <section className="next-section-padding">
        <div className="next-container text-center">
          <motion.div {...fadeInProps()} className="mx-auto max-w-3xl">
            ...
            <Button asChild size="lg" className="next-button rounded-full px-10">

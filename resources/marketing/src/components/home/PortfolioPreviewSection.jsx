...
const PortfolioPreviewSection = () => {
    ...
    const { data, loading, error } = useApi(
        '/portfolio?featured=1&home_showcase=1',
    );
    const showcaseProjects = useMemo(
        () =>
            (data?.data || [])
                .filter((project) => project.home_featured_image_url)
                .slice(0, 8),
        [data?.data],
    );
            ...
            <div className="mx-auto w-[90vw] max-w-[90vw]">
                        ...
                        <div className="scrollbar-soft overflow-x-auto pb-8">
                            <div className="mx-auto w-max px-1">
                                <div className={showcaseGridClass}>
                                    <div
                                        className={`grid grid-rows-2 gap-3 md:gap-5 ${showcaseColumnHeightClass}`}
                                    >
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-0',
                                        )}
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-1',
                                        )}
                                    </div>

                                    <div className={showcaseColumnHeightClass}>
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-2',
                                            'h-full',
                                        )}
                                    </div>

                                    <div
                                        className={`grid grid-cols-2 grid-rows-4 gap-3 md:gap-5 ${showcaseColumnHeightClass}`}
                                    >
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-3',
                                            'col-span-2 row-span-2',
                                        )}
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-4',
                                            'row-span-2',
                                        )}
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-5',
                                            'row-span-2',
                                        )}
                                    </div>
                                    ...
                                    >
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-6',
                                        )}
                                        {renderSkeletonTile(
                                            'portfolio-preview-skeleton-7',
                                        )}
                                    </div>
                                </div>
                            </div>
                ...
                {!loading && !error && !hasPortfolioProjects && (
                    <div className="next-card flex min-h-40 items-center justify-center text-center text-sm text-muted-foreground">
                        Featured projects with home showcase images will appear
                        here soon.
                    </div>
                )}

...
const FolderCard = ({ project }) => {
    ...
    const categories = Array.isArray(project.categories)
        ? project.categories
        : [];
    const types = Array.isArray(project.types) ? project.types : [];
    const client = project.client || null;
    const primaryCategory = categories[0] || null;
    const primaryType = project.primary_type || types[0] || null;

    return (
        <article className="next-card group relative flex h-full w-full max-w-full min-w-0 cursor-pointer flex-col overflow-hidden p-0">
            <Link
            ...
            <div className="flex min-w-0 flex-1 flex-col space-y-5 p-6">
                    ...
                    {client &&
                        (client.slug ? (
                            <Link
                                ...
                                className="relative z-20 inline-flex w-full max-w-full min-w-0 items-center gap-2 overflow-hidden text-xs font-medium tracking-[0.16em] text-muted-foreground uppercase transition hover:text-foreground sm:w-auto"
                            >
                                ...
                                <span className="max-w-full truncate sm:max-w-[12rem]">
                                    {client.name}
                                </span>
                            </Link>
                        ) : (
                            <div className="inline-flex w-full max-w-full min-w-0 items-center gap-2 overflow-hidden text-xs font-medium tracking-[0.16em] text-muted-foreground uppercase sm:w-auto">
                                <span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full border border-border/70 bg-muted/40">
                                    {client.logo_url ? (
                                        <img
                                            src={client.logo_url}
                ...
                <div className="min-w-0 space-y-3">
                    <h3 className="text-2xl font-semibold break-words text-foreground transition group-hover:text-primary">
                        {project.title}
                    </h3>
                    <p className="flex-1 text-sm break-words text-muted-foreground">
                        {summary}
                    </p>
                </div>
                ...
                <div className="mt-auto flex min-w-0 flex-col items-start gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
                    ...
                    <span className="pointer-events-none inline-flex h-11 w-11 items-center justify-center self-end rounded-2xl border border-border/70 bg-muted/30 text-muted-foreground transition group-hover:border-primary/40 group-hover:text-primary sm:self-auto">
